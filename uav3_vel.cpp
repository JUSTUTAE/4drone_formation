...
public:
    ...
    {
        state_sub = nh_.subscribe<mavros_msgs::State>("uav3/mavros/state", 10, &OffboardController::stateCallback, this);
        arming_client = nh_.serviceClient<mavros_msgs::CommandBool>("uav3/mavros/cmd/arming");
        set_mode_client= nh_.serviceClient<mavros_msgs::SetMode>("uav3/mavros/set_mode");
        ref_path_sub = nh_.subscribe<nav_msgs::Path>("uav3/ref_path", 10, &OffboardController::ref_pathCallback, this);
        current_pose_sub = nh_.subscribe<geometry_msgs::PoseStamped>("uav3/mavros/local_position/pose", 10, &OffboardController::poseCallback, this);

        set_vel_pub = nh_.advertise<geometry_msgs::TwistStamped>("/uav3/mavros/setpoint_velocity/cmd_vel", 10);
    }
...
{
    ros::init(argc, argv, "uav3_move_node");
